...
class MainToolbar(AbstractToolbar):
    ...
    def __init__(self, main_window: AbstractMainWindow) -> None:
        ...
        self.zoom_levels = ZoomLevels([0.25, 0.5, 0.75, 1.0, 1.5, 2.0, 4.0, 8.0, 16.0])
    ...
    def on_current_output_changed(self, index: int, prev_index: int) -> None:
        ...
        qt_silent_call(
            self.frame_control.setMaximum, self.main.current_output.end_frame
        )
...
class MainWindow(AbstractMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.display_scale = (
            self.app.primaryScreen().logicalDotsPerInch() / self.BASE_PPI
        )
    ...
    def load_script(
        ...
        try:
            ...
            exec(
                self.script_path.read_text(encoding="utf-8"), {"__file__": sys.argv[0]}
            )
    ...
    def switch_frame(
        self, pos: Union[Frame, Time], *, render_frame: bool = True
    ) -> None:
    ...
    def update_statusbar_output_info(self, output: Optional[Output] = None) -> None:
        ...
        if not output.has_alpha:
            ...
            self.statusbar.pixel_format_label.setText(
                "Clip: {}, Alpha: {}".format(
                    output.format.name, output.format_alpha.name
                )
            )
        if output.fps_den != 0:
            self.statusbar.fps_label.setText(
                "{}/{} = {:.3f} fps".format(
                    output.fps_num, output.fps_den, output.fps_num / output.fps_den
                )
            )
        else:
            self.statusbar.fps_label.setText(
    ...
    def __getstate__(self) -> Mapping[str, Any]:
        state = {
            attr_name: getattr(self, attr_name) for attr_name in self.storable_attrs
        }
        state.update(
            {
...
def main() -> None:
    ...
    main_window.load_script(
        script_path, external_args=args.external_args, start_frame=args.frame or 0
    )
    main_window.show()
...
def check_versions() -> bool:
    ...
    if sys.version_info < (3, 9, 0, "final", 0):
        logging.warning(
            "VSPreview is not tested on Python versions prior to 3.9, but you have {} {}. Use at your own risk.".format(
    ...
    if vs.core.version_number() < 53:
        logging.warning(
            "VSPreview is not tested on VapourSynth versions prior to 53, but you have {}. Use at your own risk.".format(
                vs.core.version_number()
            )
        )
        failed = True
